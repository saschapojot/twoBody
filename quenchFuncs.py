...
def Hs0_k(kVal):
    ...
    :param kVal:
    :return: Hs0 in momentum space, block kVal
    """
    ...
    for i, i_fock_str in enumerate(seed_list):
        for j, j_fock_str in enumerate(seed_list):
            ...
            retHkMat[i, j] = i_state_vec.conj().T @ Hs0 @ j_state_vec
    return retHkMat
...
def H1_k(kVal):
    ...
    :param kVal:
    :return: Hs in momentum space, block kVal
    """
    retHkMat = np.zeros((int(N_H / L), int(N_H / L)), dtype=np.complex128)
    for i, i_fock_str in enumerate(seed_list):
        for j, j_fock_str in enumerate(seed_list):
            i_state_vec = ortho_basis(kVal, i_fock_str)
            j_state_vec = ortho_basis(kVal, j_fock_str)
            retHkMat[i, j] = i_state_vec.conj().T @ H1 @ j_state_vec
    return retHkMat
...
def UMat(kVal):
    ...
    """
    return slin.expm(-1j*1/2*Tq*H1_k(kVal))@slin.expm(-1j*1/2*Tq*Hs0_k(kVal))
...
init_state=np.zeros(basisAll.Ns,dtype=np.complex128)
init_state[i0]=1
psi0Vec=init_state
